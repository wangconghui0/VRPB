...
class CVRP_Encoder(nn.Module):
    def __init__(self, **model_params):
        ...
        self.layers1 = nn.ModuleList([EncoderLayer(**model_params) for _ in range(encoder_layer_num)])
        self.layers2 = nn.ModuleList([EncoderNodeLayer(**model_params) for _ in range(encoder_layer_num)])
    ...
    def forward(self, depot_xy, node_xy_demand):
        ...
        problem_size = embedded_node.size(1)

        batch_size = embedded_node.size(0)

        deli = embedded_node[:, :self.pomo_size, :]
        pick = embedded_node[:, self.pomo_size:, :]

        out = torch.cat((embedded_depot, embedded_node), dim=1)

        for layer1 in self.layers1:
            deli, pick = layer1(deli, pick)

        for layer2 in self.layers2:
             out = layer2(out)

        addition = torch.zeros(size=(batch_size, 1, self.embedding_dim))


        out = out + torch.cat((addition, deli, pick), 1)
...
class EncoderNodeLayer(nn.Module):
    def __init__(self, **model_params):
        super().__init__()
        self.model_params = model_params
...
class EncoderLayer(nn.Module):
    def __init__(self, **model_params):
        super().__init__()
        self.deli_encoding_block = EncodingBlock(**model_params)
        self.pick_encoding_block = EncodingBlock(**model_params)

    def forward(self, deli_node, pick_node):
        # row_emb.shape: (batch, row_cnt, embedding)
        # col_emb.shape: (batch, col_cnt, embedding)
        # cost_mat.shape: (batch, row_cnt, col_cnt)
        deli_out = self.deli_encoding_block(deli_node, pick_node)
        pick_out = self.pick_encoding_block(pick_node, deli_node)

        return deli_out, pick_out
...
class EncodingBlock(nn.Module):
    ...
    def forward(self, deli_node, pick_node):
        ...
        q = reshape_by_heads(self.Wq(deli_node), head_num=head_num)
        # q shape: (batch, head_num, row_cnt, qkv_dim)
        k = reshape_by_heads(self.Wk(pick_node), head_num=head_num)
        v = reshape_by_heads(self.Wv(pick_node), head_num=head_num)
        ...
        out1 = self.add_n_normalization_1(deli_node, multi_head_out)
        out2 = self.feed_forward(out1)
        out3 = self.add_n_normalization_2(out1, out2)
